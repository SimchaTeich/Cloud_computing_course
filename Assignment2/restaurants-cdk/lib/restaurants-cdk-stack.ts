...
export class RestaurantsCdkStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const labRole = iam.Role.fromRoleArn(this, 'Role', "arn:aws:iam::174471913377:role/LabRole", { mutable: false });

    // Students TODO Account Details: Change the vpcId to the VPC ID of your existing VPC
    const vpc = ec2.Vpc.fromLookup(this, 'VPC', {
      vpcId: 'vpc-0a3c4f041ff5033bd',
    });
  ...
  private createDynamoDBTable() {
    ...
    // Create Global Secondary Index (GSI) for 'cuisine'
    table.addGlobalSecondaryIndex({
      ...
      //sortKey: { name: 'restaurantName', type: dynamodb.AttributeType.STRING },
      projectionType: dynamodb.ProjectionType.ALL, // Adjust projection type as per your needs
      readCapacity: 1, // Adjust as needed
      writeCapacity: 1, // Adjust as needed
    });

    // Create Global Secondary Index (GSI) for 'region'
    table.addGlobalSecondaryIndex({
      indexName: 'regionIndex',
      partitionKey: { name: 'regionName', type: dynamodb.AttributeType.STRING },
      sortKey: { name: 'rating', type: dynamodb.AttributeType.NUMBER },
      projectionType: dynamodb.ProjectionType.ALL, // Adjust projection type as per your needs
      readCapacity: 1, // Adjust as needed
      writeCapacity: 1, // Adjust as needed
    ...
    // Create Global Secondary Index (GSI) for 'rating'
    /*table.addGlobalSecondaryIndex({
